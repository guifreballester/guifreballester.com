...
I learned the hard way that GitOps is a culture migration wearing a tooling migration's clothes. My first instinct was a clean org-wide cutover: define the standard, announce the date, flip everyone over. I'm glad I didn't.

The moment it clicked was watching one of our most deploy-averse engineers during the pilot. He pushed a change, watched it sync in ArgoCD, then — almost as a dare to himself — deliberately broke the next one to see what would happen. ArgoCD caught it and rolled back automatically. He just sat there for a second, then said something like "oh, so I can't actually hurt anything." That was the whole project in one sentence. The blocker was never the hours; it was the fear. Once the safety net was visible and automatic, people deployed more not because we told them to, but because deploying stopped being scary.

That reframed how I ran the rollout. Instead of mandating, I started with the teams that wanted in, let the automatic rollback do the convincing, and let those early adopters carry the story to the skeptics far more credibly than any rollout plan from me could.
